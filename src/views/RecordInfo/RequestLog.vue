<template>
  <div>
    <Card>
      <CardMenuItem slot="cardTitle" title="访问日志" isActive/>
      ...
      <QueryBar slot="queryBar">
        <div slot="queryBarLeft" style="float: left;text-align: left">
          ...
          URI：<el-input v-model="queryForm.router" clearable class="query-bar-item"></el-input>
          <!-- 隐藏 窗口缩小时 控制换行 -->
      ...
      <el-table slot="contain" header-cell-class-name="table__header" row-class-name="table__row"
        ...
        <el-table-column label="URI" prop="router"></el-table-column>
...
import { getRequestLogList } from '@api/getData'
export default {
  data () {
    return {
      tableData: [], // 表格数据
